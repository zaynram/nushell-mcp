...
import { getNuMcpClient, type NuMcpToolResponse } from '#client'
import { getReplPool, NuMcpPool, parseEvaluateEnvelope } from '#pool'
...
describe('NuMcpPool — BUG 4 regression: concurrent kill and clear must not resurrect bucket', () => {
  test("kill() wins over concurrent clear('all') — bucket is gone after both settle", async () => {
    ...
    try {
      ...
      await Promise.all([p.clear('race-kill-clear', 'all'), p.kill('race-kill-clear')])
