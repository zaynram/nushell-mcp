 ...
 */
import { decodeMessage, encodeRequest, parseListCommandsOutput } from '#client'
...
describe('encodeRequest', () => {
  test('emits jsonrpc 2.0 + id + method + newline', () => {
    ...
    expect(parsed).toEqual({ jsonrpc: '2.0', id: 1, method: 'tools/list', params: {} })
  })
...
describe('decodeMessage — responses', () => {
  test('parses a success response', () => {
    const msg = decodeMessage('{"jsonrpc":"2.0","id":1,"result":{"foo":"bar"}}\n')
  ...
  test('ignores trailing whitespace and CRLF', () => {
    const msg = decodeMessage('{"jsonrpc":"2.0","id":1,"result":null}   \r\n')
    expect(msg.kind).toBe('response')
  })
})
...
describe('decodeMessage — notifications', () => {
  ...
  test('notification without params still classifies', () => {
    const msg = decodeMessage('{"jsonrpc":"2.0","method":"notifications/initialized"}')
    expect(msg.kind).toBe('notification')
    if (msg.kind !== 'notification') throw new Error('unreachable')
...
describe('decodeMessage — error paths', () => {
  ...
  test('throws on wrong jsonrpc version', () => {
    expect(() => decodeMessage('{"jsonrpc":"1.0","id":1,"result":{}}')).toThrow()
  })

  test('throws on response with neither result nor error', () => {
    expect(() => decodeMessage('{"jsonrpc":"2.0","id":1}')).toThrow()
  })

  test('throws on unclassifiable message (no id and no method)', () => {
    expect(() => decodeMessage('{"jsonrpc":"2.0","params":{}}')).toThrow()
  })
})
...
describe('parseListCommandsOutput', () => {
    ...
    expect(r).toEqual([
      {
        name: 'where',
        signature: '<condition>',
        description: 'Filter values of an input list based on a condition.',
      },
    ])
  })
  ...
  test('treats {flags} as a signature start', () => {
    const r = parseListCommandsOutput('pixi {flags} ...(args)  - Pixi - dev tool')
    expect(r[0]).toEqual({
      name: 'pixi',
  ...
  test('treats (optional) parens as a signature start', () => {
    ...
    expect(r[0]).toEqual({
      ...
      description: 'Interact with a file from an autoload directory.',
    })
  })
  ...
  test('treats [optional] brackets as a signature start', () => {
    const r = parseListCommandsOutput('foo [bar]  - test')
    expect(r[0]).toEqual({ name: 'foo', signature: '[bar]', description: 'test' })
  })

  test('line with no description (no separator)', () => {
    const r = parseListCommandsOutput('foo <bar>')
    expect(r[0]).toEqual({ name: 'foo', signature: '<bar>', description: null })
  })
  ...
  test('multiple lines parsed together preserve order', () => {
    const text = 'where <condition>  - Filter\npolars arg-true  - Returns indexes'
  ...
  test('only-name line (no signature, no description)', () => {
    ...
    expect(r[0]).toEqual({ name: '$nu', signature: null, description: null })
  })
})
