...
describe('MCP server wiring', () => {
  test('server initializes over stdio and lists its twelve tools', async () => {
    ...
    try {
      ...
      send({
        ...
        id: 3,
        method: 'tools/call',
        params: { name: 'nu_doc_search', arguments: { query: 'where', limit: 3 } },
      })
      send({
        jsonrpc: '2.0',
        id: 4,
        method: 'tools/call',
        params: { name: 'nu_doc_help', arguments: { name: 'where' } },
      })
      await proc.stdin.flush()
  ...
  test('nu_repl_spawn / nu_repl_list / nu_repl_kill / nu_repl_nuke', async () => {
    ...
    try {
      ...
      expect((r9.result.structuredContent?.killed as number) ?? 0).toBeGreaterThanOrEqual(
        2
      )
      ...
      const w1 = await call('nu_repl_write', { key: 'writebench', input: 'let x = 42' })
      ...
      const wErr = await call('nu_repl_write', { key: 'no-such-bucket', input: '1' })
      ...
      expect(
        (rd1.result.structuredContent?.response as { text?: string } | null)?.text ?? ''
      ).toContain('101')
      ...
      expect((yLookup.result.structuredContent?.output as string) ?? '').toContain('99')
      ...
      send({
        ...
        params: {
          name: 'nu_repl_write',
          arguments: { key: 'iso1', input: "sleep 400ms; 'one'" },
        },
      })
      send({
        jsonrpc: '2.0',
        ...
        params: {
          name: 'nu_repl_write',
          arguments: { key: 'iso2', input: "sleep 400ms; 'two'" },
        },
      })
      await proc.stdin.flush()
  ...
  test('nu_exec: [bashEnv runner: ...] label appears in MCP content text', async () => {
    if (!(await bashRuntimeAvailable())) {
      console.warn('skipping bashEnv runner label MCP test — no bash runtime detected')
      return
    }
    ...
    try {
      ...
      send({
        ...
        id: 3,
        method: 'tools/call',
        params: { name: 'nu_exec', arguments: { pipeline: '1 + 1' } },
      })
      await proc.stdin.flush()
