...
import { type BucketStatus, getReplPool } from '#pool'
...
function renderExec(
  ...
  if (resultType && resultType !== 'nothing') parts.push(`\n[result type: ${resultType}]`)
...
server.registerTool(
  ...
  async ({
    ...
    const effectiveTimeout = timeoutMs ?? vars.TIMEOUT_MS
    try {
...
server.registerTool(
  ...
  async ({ query, limit }) => {
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      return {
        content: [{ type: 'text', text: `Documentation search failed: ${message}` }],
        isError: true,
      }
    }
  }
...
server.registerTool(
  ...
  async ({ name }) => {
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      return {
        content: [{ type: 'text', text: `Documentation lookup failed: ${message}` }],
        isError: true,
      }
    }
  }
...
server.registerTool(
  ...
  async ({ key, mode }) => {
    ...
    try {
      ...
      return {
        content: [
          { type: 'text', text: `Cleared bucket "${key}" (mode: ${effective}).` },
        ],
      ...
      return {
        ...
        structuredContent: {
          ...
          ...(cached.kind === 'ok' && {
            ...
            ...(cached.timestamp !== undefined && { timestamp: cached.timestamp }),
          }),
          envKeys: [],
          probeError: status.probeError,
        },
...
server.registerTool(
  ...
  async () => {
    ...
    return {
      content: [{ type: 'text', text: `Aborted ${aborted} exec/bash subprocess(es).` }],
...
console.error(`nushell-mcp running on stdio (nu ${version} at ${vars.NU_PATH})`)
